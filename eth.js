...
const ETHERSCAN_API_KEY = '2S8HBB49TRGD3APYD6U2FNTUYYUG6HP6BF';
  ...
  while (blockProcessStart <= toBlockNum) {
    try {
      ...
      if (blockLogs.length >= 1000) {
        ...
        process.stdout.write(`Fetching contract ETH TXS from block ${blockProcessStart} to block ${blockProcessEnd}. Remaining Blocks: ${Number(toBlockNum - blockProcessEnd)}`);
...
async function start(contract) {
  let balances = [];
  try {
    const blockNum = await getETHRPCResponse('eth_blockNumber');
    const transfers = await getContractTransfers(contract, 12643111, blockNum); // 12643111, contract created on 12643112
