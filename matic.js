...
const POLYGONSCAN_API_KEY = 'GWGDXHH4N9ZMG89V3KX7ZMNNSEF2VZHG79';

const urls = [
  'https://winter-black-energy.matic.quiknode.pro',
];
...
async function getMATICRPCResponse(method, params = []) {
  ...
  while (blockProcessStart <= toBlockNum) {
    try {
      ...
      if (blockLogs.length >= 1000) {
        ...
        process.stdout.write(`Fetching contract MATIC TXS from block ${blockProcessStart} to block ${blockProcessEnd}. Remaining Blocks: ${Number(toBlockNum - blockProcessEnd)}`);
...
async function start(contract) {
  let balances = [];
  try {
    const blockNum = await getMATICRPCResponse('eth_blockNumber');
    const transfers = await getContractTransfers(contract, 43214000, blockNum); // 43214000, contract created on 43214021
  ...
  const filepath = `${homeDirPath}maticlist.json`;
  console.log(`Done MATIC - ${balances.length} records found. Exported at ${filepath}. Total Flux-MATIC: ${total.toLocaleString()} (${total}).`);
